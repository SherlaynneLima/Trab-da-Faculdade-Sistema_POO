...
public class TelaAgenda extends JFrame {
	...
	public TelaAgenda() {
		...
		btnConfirmarAgendamento.setFont(new Font("Dialog", Font.BOLD, 13));
		btnConfirmarAgendamento.setBackground(new Color(152, 251, 152));
		btnConfirmarAgendamento.setBounds(471, 383, 159, 35);
		...
		JButton btnVerAgendamentos = new JButton("Ver Agendamentos");
		btnVerAgendamentos.setForeground(new Color(0, 0, 139));
		btnVerAgendamentos.setFont(new Font("Dialog", Font.BOLD, 13));
		btnVerAgendamentos.setBackground(new Color(173, 216, 230));
		btnVerAgendamentos.setBounds(471, 429, 159, 35);
		JanelaAgendamento.add(btnVerAgendamentos);
		
		// btnVerAgendamentos
        btnVerAgendamentos.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                TelaListaAgendamentos telaListaAgendamentos = new TelaListaAgendamentos();
                telaListaAgendamentos.setVisible(true);
            }
        });
        ...
        btnConfirmarAgendamento.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String unhas = chckbxUnhasAgendamento.isSelected() ? chckbxUnhasAgendamento.getText() : "";
                String cabelo = chckbxcabeloCorteprocedimentos.isSelected() ? chckbxcabeloCorteprocedimentos.getText() : "";
                String maquiagem = chckbxMaquiagemAgendamento.isSelected() ? chckbxMaquiagemAgendamento.getText() : "";
                String design = chckbxdesignDesobrancelhas.isSelected() ? chckbxdesignDesobrancelhas.getText() : "";
                String estetica = chckbxesteticaFacialcorporal.isSelected() ? chckbxesteticaFacialcorporal.getText() : "";
                String diaNoiva = chckbxdiaDanoiva.isSelected() ? chckbxdiaDanoiva.getText() : "";
                
                Agendamentos agendamentos = new Agendamentos(
                    cbxListaClientes.getSelectedItem().toString(),
                    cbxData.getSelectedItem().toString(),
                    cbxHorario.getSelectedItem().toString(),
                    cbxFormaPgmt.getSelectedItem().toString(),
                    cbxAtendente.getSelectedItem().toString(),
                    unhas,
                    cabelo,
                    maquiagem,
                    design,
                    estetica,
                    diaNoiva,
                    txtAreaDescricaoAgendamento.getText()
                );

                Agendamentos.adicionarAgendamentos(agendamentos);
            ...
            private void limparCampos() {
        	    chckbxUnhasAgendamento.setSelected(false);
        	    chckbxcabeloCorteprocedimentos.setSelected(false);
        	    chckbxMaquiagemAgendamento.setSelected(false);
        	    chckbxdesignDesobrancelhas.setSelected(false);
        	    chckbxesteticaFacialcorporal.setSelected(false);
        	    chckbxdiaDanoiva.setSelected(false);
        	    txtAreaDescricaoAgendamento.setText("");
